...
import { Button, Card, CardBody, Col, Row, Container, UncontrolledTooltip } from 'reactstrap'; // Thêm Container vào để bọc lưới
...
const InfoFolder = ({ id, author, title, handleDeleteInfo, handleEditInfo }) => {
    ...
    return (
        <div>
            ...
            <Col style={{ marginTop: '-25px', marginTop: '25px' }}>
                <Button.Ripple className={styles.width_button_auto} color="primary" onClick={openCreateModal} style={{ marginLeft: '10px' }}>
                    <Plus size={14} />
                    <span className="align-middle ms-25">{t('fieldName.addStudySet')}</span>
                </Button.Ripple>
            </Col>
                    ...
                    {studySets.map((studySet) => (
                        ...
                        <Col sm="6" key={studySet._id}>
                            <Link to={`${linkToFlashCard}/${studySet._id}`}>
                                <Card style={{ height: '100px', marginTop: '10px' }} className={`${styles.card_infoFolder}`}>
                        ...
                        {`${studySet?.numberOfTerms} ${t('fieldName.terms')}`}
                                            ...
                                            <div className={`${styles.author} d-flex align-items-center cursor-pointer`} style={{ marginLeft: '1rem' }}>
                                                <Avatar
                                                    size="sm"
                                                    img={author?.avatar || '/src/assets/images/portrait/small/avatar-s-11.jpg'}
                                                />
                                                <h5 style={{ margin: '0 0 0 .5rem' }}>{studySet.title}</h5>
                                            </div>
                                        </div>
                                        <Row className="mt-2">
                                            <div style={{ display: "flex", flexDirection: "column" }}>
                                                ...
                                                >
                                                    {studySet.description.length > 65 ? `${studySet.description.slice(0, 65)}...` : studySet.description}
                                                </h6>
                                                <UncontrolledTooltip target={`title-tooltip-${studySet.id}`} placement="top">
            ...
            <ModalAddStudySetToFolder folderId={params.inforFolder} isOpen={showCreateModal} onClose={() => setShowCreateModal(false)} isUpdated={isUpdated => { getStudySetByFolderId() }} />
        </div>
    );
};
